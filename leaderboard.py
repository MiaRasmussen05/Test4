"""
File for the leader board top 20
"""
...
def create_players(scores):
    """
    Function to create a list of player objects
...
def calculate_max_lengths(players):
    """
...
def display_leaderboard():
    ...
    def get_player_score(player):
        if player.score.isdigit():
            return int(player.score)
        else:
            return 0

    sorted_scores = sorted(players, key=get_player_score, reverse=True)

    top_score = sorted_scores[:15]
    header_length = len(f"|| {'NAME':<29} || {'SCORE':^4} ||")

    print("||" + "=" * header_length + "||")
    print(f"|| {'NAME':<33} || {'SCORE':^5} ||")
    print("||" + "=" * header_length + "||")
